"""Augment a SeoulMOD GU-level dataset with origin/destination hourly rainfall channels.

Pipeline:
  1. Load 12 monthly Seoul AWS rain CSVs (10-min interval, per-station)
     for the requested year.
  2. Aggregate to (gu_name, hour) hourly rainfall (mm/h):
       - sum the 6 ten-minute readings per station-hour,
       - mean across stations within the gu.
  3. Map gu_name (English) -> 5-digit admin code via gu_centroids.csv.
  4. For each (origin_gu, dest_gu, hour) bucket, attach (rain_o, rain_d).
...
Output: datasets/SeoulMOD_GU_{year}_rain/
"""
...
def load_rain_year(rain_glob: str) -> pd.DataFrame:
    ...
    for f in files:
        ...
        df.columns = ['station_code', 'station_name', 'gu_code', 'gu_name_en', 'rain_10min', 'ts']
        dfs.append(df[['station_name', 'gu_name_en', 'rain_10min', 'ts']])
...
def aggregate_to_gu_hour(rain: pd.DataFrame) -> pd.DataFrame:
    print('Aggregating to (gu_name, date, hour) hourly mm...')
    per_station = (
        rain.groupby(['station_name', 'gu_name_en', 'date', 'hour'], as_index=False)
        ['rain_10min'].sum()
    )
    per_gu_hour = (
        per_station.groupby(['gu_name_en', 'date', 'hour'], as_index=False)
        ['rain_10min'].mean()
    )
    per_gu_hour = per_gu_hour.rename(columns={'rain_10min': 'rain_mm'})
    print(f'  {len(per_gu_hour):,} (gu, date, hour) rows; '
          f'rain_mm mean={per_gu_hour.rain_mm.mean():.4f}, max={per_gu_hour.rain_mm.max():.2f}')
    return per_gu_hour
...
def main() -> None:
    ...
    out = REPO / 'datasets' / f'SeoulMOD_GU_{args.year}_rain'
    rain_glob = str(REPO / 'datasets' / 'rainfall' / f'seoul_rain_{args.year}*.csv')
    gu_cent_path = REPO / 'datasets' / 'gu_centroids.csv'
    ...
    gu_cent = pd.read_csv(gu_cent_path)
    gu_cent['gu_cd_str'] = gu_cent['gu_cd'].astype(str)
    name_to_cd = dict(zip(gu_cent['gu_name'], gu_cent['gu_cd_str']))
    print(f'name_to_cd: {len(name_to_cd)} mappings')

    all_gus = sorted(name_to_cd.values())
    gu_idx = {cd: i for i, cd in enumerate(all_gus)}
    print(f'gu order (first 5): {all_gus[:5]}, count: {len(all_gus)}')

    rain = load_rain_year(rain_glob)
    per_gu_hour = aggregate_to_gu_hour(rain)

    base = Date(args.year, 1, 1)
    rain_mat = np.zeros((T, len(all_gus)), dtype=np.float32)
    n_filled = 0
    for _, row in per_gu_hour.iterrows():
        gu_name = row['gu_name_en']
        if gu_name not in name_to_cd:
            continue
        cd = name_to_cd[gu_name]
        if cd not in gu_idx:
            continue
        gi = gu_idx[cd]
    ...
    print(f'rain matrix filled: {n_filled:,} '
          f'(out of {T*len(all_gus):,} cells, {100*n_filled/(T*len(all_gus)):.1f}%)')
    print(f'  per-gu mean: {rain_mat.mean(axis=0)[:5]}, overall mean: {rain_mat.mean():.4f}')

    rain_pair = np.zeros((T, len(pairs), 2), dtype=np.float32)
    for idx, (o, d) in enumerate(pairs):
        if o not in gu_idx or d not in gu_idx:
            print(f'  WARN: pair {idx} ({o},{d}) gu not in mapping')
            continue
        rain_pair[:, idx, 0] = rain_mat[:, gu_idx[o]]
        rain_pair[:, idx, 1] = rain_mat[:, gu_idx[d]]
    ...
    new_desc = dict(desc)
    new_desc['name'] = f'SeoulMOD_GU_{args.year}_rain'
