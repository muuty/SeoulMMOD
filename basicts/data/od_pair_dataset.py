...
indexes individual OD pairs and returns [L, N_modes, 1] per sample.
...
class ODPairDataset(BaseDataset):
    """Dataset indexed by (time, od_pair) for per-pair forecasting."""
    ...
    def __len__(self) -> int:
        return self.n_time_samples * self.n_pairs

    def __getitem__(self, index: int) -> dict:
        # Decompose flat index into (time_idx, pair_idx).
        time_local = index // self.n_pairs
