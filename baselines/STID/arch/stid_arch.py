...
class STID(nn.Module):
    ...
    def __init__(self, **model_args):
        ...
        # tod/dow channel indices (default to original PEMS layout: ch 1, 2)
        ...
        # regression: output_len * output_dim channels, reshaped in forward
    ...
    def forward(self, history_data: torch.Tensor, future_data: torch.Tensor, batch_seen: int, epoch: int, train: bool, **kwargs) -> torch.Tensor:
        ...
        # regression: [B, output_len*output_dim, N, 1] -> [B, output_len, N, output_dim]
        prediction = self.regression_layer(hidden)
        if self.output_dim > 1:
            B, _, N, _ = prediction.shape
            prediction = prediction.view(B, self.output_len, self.output_dim, N).permute(0, 1, 3, 2)
        # else: keep [B, output_len, N, 1] (backward compat with single-channel)
